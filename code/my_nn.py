...
class MyNN:
    ...
    def __init__(self, hl_1_units, hl_2_units, learning_rate, momentum, nesterov, optimizer):
    ...
    def get_optimizer(self):
        opt = self.hyperparameters["optimizer"]
        if opt == Optimizer.ADAM:
            return torch.optim.Adam(self.model.parameters(), lr=self.hyperparameters["learning_rate"])
        elif opt == Optimizer.SGD:
            return torch.optim.SGD(self.model.parameters(), lr=self.hyperparameters["learning_rate"], momentum=0.9)
        elif opt == Optimizer.RMSPROP:
            return torch.optim.RMSprop(self.model.parameters(), lr=self.hyperparameters["learning_rate"],
    ...
    def get_model_unique_name(self):
        return "nn_" + str(self.hyperparameters)
...
def main():
    ...
    for hl_units in hl_values:
        nn = MyNN(hl_units, 200, 0.001, 0.9, False, Optimizer.RMSPROP)
        num_epochs = 300  # the best obtained
        ...
        #                                   False, "cnn_" + str(num_epochs) + "_" + str(i + 1))
